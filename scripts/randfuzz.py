"""
=====================
This is an automated random fuzzing tool for SmartThings Apps
@author: lkshar
=====================
Run command: (test jesper : VPS_APP and set waiting time aft an event to 60 sec)
$ source apsecenv/bin/activate
$ python randomfuzz.py <Your Smart App Name> <Your Samsung Account> <Your Samsung Account Password>
"""
print(__doc__)
import os, sys
import datetime
...
def add_to_Params(param, values, dtype=""):
    #if not values:  # if there is no value, skip
    #    return
...
def run_analysis(randomise):
    ...
    simulate_events(randomise)

    time.sleep(3)
    
# simulate input values in Preferences/sections
def simulate_inputs(randomise):
    # get Preferences section
    elem = driver.find_element(By.ID, "ide-preferences-panel")
    # each user input is under a section class
    sections = elem.find_elements(By.CLASS_NAME, "section")
    #sections = elem.find_elements_by_class_name("section")
    ...
    for section in sections:
        ...
        for i,input_class in enumerate(input_classes):
            ...
            try: 
                for elem in input_tags:
                    process_input(elem,randomise)
                    
                for elem in select_tags:
                    process_select(elem,randomise)
            except:
...
def process_input(elem,randomise):
    input_type = elem.get_attribute("type")
    ...
    if param in params:
        idx = params.index(param)
        value = randomise[idx] # the value comes from randomise test gen

        if param in default_params: # randomly get a different value for params assigned with default values initially
            if utility.change():
                # randfuzz doesn't analyze source code and thus, it doesn't have dtype information (from data file)
    ...
    if input_type == "checkbox" or input_type == "radio" or input_type == "button" or input_type == "reset" or input_type == "submit":
        ...
        return # currently we don't deal with file input type 
    else:
        try:
            elem.clear()
...
def process_select(elem,randomise):
    elem.click()
    param = elem.get_attribute("id")
    value = ""
    if param in params:
...
# idea: simulate param in params that corresponds to capability or endpoint, in a random order, till all relevant params are simulated
def simulate_events(randomise):
    ...
    for rand_idx in rand_indices:
        ...
        idx = params.index(param)
        value = randomise[idx]  # get corresponding value for the param, from randomise test gen
...
def record_event(param,value): # send event information to our backend server
    ...
    try:
        #ep_url = 'https://62f870a15b61.ngrok.io/fuzzthings/things.php'
...
def log_duration_sinkscovered(): # log duration and sink covered after running each test case
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='app_ranfuzz.log', filemode='a', format='%(message)s', level=logging.INFO)
    logging.info("Random fuzzing App: " + appName)
    record_event("App", appName)

    print("===========Random fuzzing App: " + appName + "===============")

    # initialize parameters 
    sinks = { "num_sinks" : 0, "num_sinks_covered" : 0, "sinks_covered" : [] }
    tests = { "num_testcases" : 0, "timeout" : 2, "isTimeout" : False, "isAllSinksCovered" : False}
    ...
    while not tests["isTimeout"]:
        randomise = random_fuzz()
        ...
        log_duration_sinkscovered()

    if tests["isTimeout"] or tests["isAllSinksCovered"]:
        if tests["isTimeout"]:
            print("Timeout")
