...
def sendText() {
	...
	if (location.contactBookEnabled) {
		...
		data['action'+cnt] = "sink1 : sendNotificationToContacts ${message} Your ride is here! ${recipients}"  // instrumented code
		sendNotificationToContacts(message ?: "Your ride is here!", recipients)
	}
	else {
		cnt++ // instrumented code
		data['action'+cnt] = "sink2 : sendSms ${phoneNumber} ${message} Your ride is here!"  // instrumented code
