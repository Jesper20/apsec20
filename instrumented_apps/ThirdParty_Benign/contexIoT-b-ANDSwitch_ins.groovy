...
def switchHandler(evt) {
    ...
    if ("$sw1cur" == switch1state.toLowerCase() && "$sw2cur" == switch2state.toLowerCase()) {
        ...
        if (switchOutState == "On") {
            ...
            data['action'+cnt] = "sink1: SwitchOut On"  // instrumented code
            switchOut.on()
        } else if (switch2Reset == "Yes") {
            log.debug "SwitchOut Off"
            cnt++ // instrumented code
            data['action'+cnt] = "sink2: SwitchOut Off"  // instrumented code
            switchOut.off()
        }
    } else if (switch2Reset == "Yes") {
        ...
        if (switchOutState == "On") {
            log.debug "SwitchOut Off"
            cnt++ // instrumented code
            data['action'+cnt] = "sink3: SwitchOut Off"  // instrumented code
            switchOut.off()
        } else {
            log.debug "SwitchOut On"
            cnt++ // instrumented code
            data['action'+cnt] = "sink4: SwitchOut On"  // instrumented code
            switchOut.on()
        }
    }
