 ...
 */
definition(
    name: "Multiple devices 1",
    namespace: "CSL",
    author: "Amit Sikder",
    updated: "Leo Babun",
    description: "A variable declared in the app is used in different connected devices and leaked via sinks. A variable is declared and used in different devices in line 105 and line 159 and finally leaked in line 183 via hard-coded phone number",
    category: "Mode Magic",
...
private takeActions() {
	...
	def message = "Mode is changed to sleep"
	send(message)
	cnt++
...
private send(msg) {
	def data = [:]  // instrumented code
	def cnt = 0 // instrumented code
	def phonenumber = "(111)-111-1111"  // this is the attacking point: hard-coded phone number
    if (location.contactBookEnabled) {
        ...
        if (phonenumber) {
			...
			cnt++
        	data["action"+cnt] = "sink4: sendSms $phonenumber $msg"
